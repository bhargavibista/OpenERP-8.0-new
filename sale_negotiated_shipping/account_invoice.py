...
#                
    ...
    def finalize_invoice_move_lines(self, cr, uid, invoice, move_lines):
        ...
        if invoice.sale_account_id and invoice.shipcharge:
            lines1 = {
            ...
            for move_line in move_lines:
                ...
                if journal_entry['account_id'] == invoice.journal_id.default_debit_account_id.id:
    ...
    def _compute_amount(self):
        ...
        print"self.amount_untaxed",self.amount_untaxed
        self.amount_tax = sum(line.amount for line in self.tax_line)
        self.amount_total = self.amount_untaxed + self.amount_tax
        print"self.amount_untaxed",self.amount_tax
        print"self.amount_untaxed",self.amount_total
#        self.amount_total= (self.shipcharge if self.ship_method_id else 0.0) 
#        for invoice in self:
#            if invoice.ship_method_id:
#                self.amount_total += invoice.shipcharge
    ...
    amount_total = fields.Float(string='Total', digits=dp.get_precision('Account'),
        store=True, readonly=True, compute='_compute_amount')
#    _columns = {
#        'amount_untaxed': fields.function(_compute_amount, method=True, digits_compute=dp.get_precision('Account'), string='Subtotal',
#            store = True, multi='all'),
#        'amount_tax': fields.function(_compute_amount, method=True, digits_compute=dp.get_precision('Account'), string='Tax',
#            store = True, multi='all'),
