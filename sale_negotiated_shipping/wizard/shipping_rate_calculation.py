...
#            cr.execute('select type,id from res_partner_address where partner_id = %s', (tuple([model_obj.partner_id.id])))
            cr.execute('select type,id from res_partner where id = %s', (tuple([model_obj.partner_id.id])))
            res = cr.fetchall()
            address = dict(res)
            if address:
