...
#        print"values--------------",values
        if code:
            ...
            print "app_unlink====",auth_user_id
            if auth_user_id:
                partner=self.browse(cr,uid,auth_user_id[0]).name
                if partner:
...
#        'partner':fields.many2one('res.partner','Partner'),
    ...
    def link_account(self, cr, uid, dict, context=None):
        ...
        if auth_user_id==[]:
            ...
            return { "code":False, "message":"No Such Device"}
        if auth_user_id and partner_id:
            self.write(cr,uid,auth_user_id,{'name':partner_id})

            pat_obj=self.pool.get('res.partner').browse(cr,uid,partner_id)
            accountPin= pat_obj.pin
            full_name=pat_obj.name
            x=full_name.find(' ')
            full_name=full.replace(' ','')
        ...
        return { "code":False, "message":"Partner Not Present"}
