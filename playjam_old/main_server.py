...
class Home(openerpweb.Controller):
    ...
    def activate_old(self,req,deviceId,wantCode):
        result={}
        print "aaaaaaaaaa------",req
	print"reqqqqqq",req.httprequest
        print "1111111111",deviceId
        print "wc----------",wantCode
        osv_pool = pooler.get_pool('playjam')
        user = osv_pool.get('user.auth')
#        result=user.get_key_code()
        registry = openerp.modules.registry.Registry('playjam')
        with registry.cursor() as cr:
            result=user.get_key_code(cr,1,deviceId,wantCode,{})
...
#        ero
    ...
    @openerpweb.jsonrequest
    def login(self,req,deviceId,authReply):
        result={}
        print "aaaaaaaaaa------",req
        print "1111111111",deviceId
        print "wc----------",authReply
...
#        result=user.get_key_code()
        registry = openerp.modules.registry.Registry('playjam')
        with registry.cursor() as cr:
            result=user.user_login(cr,1,deviceId,authReply,{})
