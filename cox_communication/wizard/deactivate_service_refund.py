...
class refund_customer_payment(osv.osv_memory):
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        result['linked_refund'] = False
        return_object = self.pool.get('return.order')
        res_partner_obj = self.pool.get('res.partner')
        active_id = context.get('active_id',False)
        partner_id=return_object.browse(cr,uid,active_id).partner_id.id
        cr.execute("select profile_id,credit_card_no from custmer_payment_profile where customer_profile_id='%s' and active_payment_profile=True"%(str(res_partner_obj.browse(cr,uid,partner_id).customer_profile_id)))
        payment_profile_data=cr.dictfetchall()
        if payment_profile_data:
            result['active_payment_profile']=payment_profile_data[0].get('profile_id')
            result['active_cc_number'] = payment_profile_data[0].get('credit_card_no')
        return result
    ...
    def refund_customer(self,cr,uid,ids,context={}):
        ...
        if active_id:
            ...
            if config_ids:
                ...
                cc_number = wizard_obj.cc_number
                if cust_payment_profile_id==linked_payment_profile_id:
...
#            maxmind api call to check for restricting prpeaid cards by bhargavi
                            ...
                            cust_payment_profile_id=linked_payment_profile_id
