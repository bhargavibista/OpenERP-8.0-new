...
#                new lines added
                ...
                count_package = 0
                if not use_mps:
                    ship_id.similar_packages=[0]
                for line in ship_id.similar_packages:
                    package_weight = line.package_dim.weight
...
#                package1_weight.Value = ship_id.weight_package #1.0
                ...
                if use_mps:
                    package1_weight.Value = package_weight
                    pack_dim = package_dim
...
#                    'res_model':'stock.picking.out' ,
		    'res_model':'stock.picking.out' if picking.type=="out" else 'stock.picking',	
                    'res_id': picking.id,
                }
#                fedex_attach_id = fedex_attachment_pool.search(cr,uid,[('res_id','=',picking.id),('res_name','=',picking.name)])
...
#                    fedex_attach_id = fedex_attach_id[0]
                ...
                if fedexTrackingNumber:
                    ...
                    write_result = picking.write(vals)

                    context.update({'active_id':picking.id, 'active_ids':[picking.id],'active_model':'stock.picking.out' if picking.type=="out" else 'stock.picking'})
#                    partial_id = self.pool.get("stock.partial.picking").create(cr, uid, {}, context=context)
#                    context.update({'partial_id':partial_id})
#                    picking_obj.make_picking_done(cr,uid,[picking.id],context)
#            else:
 #               raise osv.except_osv(_('Warning !'),_("This shipping quotes has been already accepted"))
	...
	#print "context in return.py file",context
