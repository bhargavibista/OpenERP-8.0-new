...
WARNING_TYPES = [('warning',' '),('info','Information'),('error','Error')]
...
class warning(osv.osv_memory):
    ...
    def _get_view_id(self, cr, uid):
        ...
        """
        res = self.pool.get('ir.model.data').get_object_reference(cr, uid,'cox_communication', 'warning_form')
        return res and res[1] or False

    def message_info(self, cr, uid, id, context):
    ...
    def warning_message(self, cr, uid, title, message, context=None):
        id = self.create(cr, uid, {'title': title, 'message': message, 'type': 'warning'})
        res = self.message_info(cr, uid, id, context)
        return res
