...
#		    print "result",result,user_id,password,hash_pwd,website_ids	
                    ### create client log records
                    if result:
                        vals={
                            'partner_id':search_partner[0] if search_partner else False,
                            'user_id':user_id,
                            'username_match':result.get('username_match'),
                            'password_match':result.get('password_match'),
                            'encrypted_password':result.get('encrypted_password'),
                            'login_time':login_time.strftime('%Y-%m-%d %H:%M:%S'),
                            'customer_id':result.get('customer_id'),
                        }
#			print "vals",vals
                        new_id = partner_log_obj.create(cr,uid,vals)
#			print "new",new_id
...
#    'customer_payment_profile_id': fields.related('sale_order_related','customer_payment_profile_id',type='char',string='Payment Profile ID',store=True,readonly=True),
    ...
    'free_trial_date': fields.date('Free Trail Date', select=True, help="Date till which free trial is given"),
...
class partner_payment_error(osv.osv):
    ...
    def weekly_exceptions_charge(self,cr,uid,ids):
        ...
        transaction_id=''
        current_obj= self.browse(cr,uid,ids)
        invoice_id=current_obj.invoice_id.id
        if invoice_id:
        ...
        if partner_id:
            ...
            if config_ids and customer_profile_id:
                ...
                if transaction_details and transaction_details.get('resultCode',False) == 'Ok' and act_model=='account.invoice':
                    ...
                    if current_obj.invoice_id.state =='draft':
                        ...
                        so_obj.email_to_customer(cr,uid,current_obj.invoice_id,'account.invoice','',current_obj.invoice_id.partner_id.emailid,context)
                        sale_name=current_obj.invoice_name.replace('RB','').split('|')
#                    code to update suspension date to False when payment is done
                        if sale_name:
                            for each_name in sale_name:
                                ...
                                partner_obj.write(cr,uid,current_obj.partner_id.id,{'billing_date':nextmonth})
                    ...
                    if policy_id:
                        for each in policy_obj.browse(cr,uid,policy_id):
                            ...
                            if cancel_date and sus_date:
                                ...
                                cmpr_date=each_exception.next_retry_date
                            if next_retry_date<=cmpr_date:
                                transaction_details=self.weekly_exceptions_charge(cr,uid,each_exception.id)
                                if transaction_details and transaction_details.get('resultCode',False) == 'Ok':
                                    ...
                                    next_retry_date=today+datetime.timedelta(weeks=1)
                                    if next_retry_date<=cmpr_date:
                                       next_try_date= next_retry_date
                                    else:
                                       next_try_date=False
...
#        function to update the retry date of exceptions whose active payment is true
    def next_retry_update(self,cr,uid,ids,context):
        policy_obj=self.pool.get('res.partner.policy')
        active_exceptions = self.search(cr,uid,[('active_payment','=',True)])
        if active_exceptions:
            for each_exception in self.browse(cr,uid,active_exceptions):
...
#                            cancellation_data = return_obj.additional_info(cr,uid,additional_info)
            ...
            self.write(cr,uid,pay_error.id,{'active_payment':False})
        if final_dict:
            referential_obj = self.pool.get('external.referential')
...
class res_partner_auth_log(osv.osv):
    ...
    def export_auth_log(self,cr,uid,ids,context=None):
        ...
        if auth_ids:
            ...
            ,"+"User Name Match"+","+"Password Match"+","+"Login Time"+","+"Login Error"+"\n"
            f.write(datas)
            datas=""
            for  each_id in self.browse(cr,uid,auth_ids):
                 datas=str(each_id.partner_id.name)+","+str(each_id.customer_id)+","+str(each_id.user_id)+"\
                 ,"+str(each_id.encrypted_password)+","+str(each_id.username_match)+","+str(each_id.password_match)+"\
                 ,"+str(each_id.login_time)+","+str(each_id.login_error)+"\n"
                 f.write(datas)
