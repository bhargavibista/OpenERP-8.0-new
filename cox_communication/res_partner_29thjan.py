...
#	today=datetime.date.today()
        ...
        if partner_id:
            ...
            cr.execute("select profile_id from custmer_payment_profile where customer_profile_id='%s' and active_payment_profile=True"%(str(res_obj.browse(cr,uid,partner_id).customer_profile_id)))
            ...
            if config_ids and customer_profile_id:
                ...
                if transaction_details and transaction_details.get('resultCode') == 'Ok' and act_model=='account.invoice':
...
#                    code to update suspension date to False when payment is done
                        if sale_name:
                            for each_name in sale_name:
                                ...
                                partner_obj.write(cr,uid,current_obj.partner_id.id,{'billing_date':nextmonth},context)
                ...
                if invoice_origin:
                    ...
                    if policy_id:
                        ...
                        if cancel_date and sus_date:
                            ...
                            print "cancel_datecancel_date",cancel_date
                            cmpr_date=datetime.datetime.strptime(cancel_date, "%Y-%m-%d")
                        ...
                        if next_retry_date<=cmpr_date:
                            ...
                            invoice_date=each_exception.invoice_date
                            if transaction_details and transaction_details.get('resultCode') == 'Ok':
                                self.write(cr,uid,each_exception.id,{'active_payment':False,'next_retry_date':False})
                            else:
                                transaction_message=transaction_details.get('message')
                                ...
                                self.write(cr,uid,each_exception.id,{'message':str(transaction_message)})
                        return invoice_date
