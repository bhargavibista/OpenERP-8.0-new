        ...
        if product:
            ...
            if product_id_obj.type=='service' and product_id_obj.recurring_service==True:
                if date_order:
                    ...
                    res['value']['end_date']=str(datetime.strptime(date_order, "%Y-%m-%d")+relativedelta(months=24))
...
#        today=datetime.strptime(today, "%Y-%m-%d")
        if not billing_date:
            cr.execute("update res_partner_policy set active_service=False,cancel_date=%s,additional_info=%s,return_cancel_reason=%s where id=%s",(time.strftime('%Y-%m-%d'),cancellation_reason,main_reason,service_id.id))
            res['state'] = 'done'
            return res
...
#		if (context and context.get('refund_cancel_service')):
		if (context and  (context.get('refund_cancel_service') or (context.get('active_model')=='credit.service'))) or (context and context.get('immediate_cancel')):
			...
			if (not service_id.return_date):
	                        cr.execute("update res_partner_policy set active_service=False,return_date=%s,additional_info=%s,return_cancel_reason=%s where id=%s",(time.strftime('%Y-%m-%d'),cancellation_reason,main_reason,service_id.id))
...
#                self.log(cr,uid,service_id.id,"Service %s Will be Cancelled on %s"%(service_name,cancellation_date))
  ...
  #              'cancellation_reason': (credit_line.notes if credit_line else ''),
		...
		cr.execute("update res_partner_policy set cancel_date=%s,additional_info=%s,return_cancel_reason=%s where id=%s",(cancellation_date,cancellation_reason,main_reason,service_id.id))	
		res['state'] = 'done'
		return_obj.update_billing_date(cr,uid,service_id.agmnt_partner.id,billing_date,service_id.sale_line_id) 	
#            self.log(cr,uid,service_id.id,"Service %s Will be Cancelled on %s"%(service_name,cancellation_date))
        return res
