            ...
            CREATE OR REPLACE view report_stock_move AS (
                SELECT
                    ...
                    FROM
                        ...
                        LEFT JOIN ir_property ip ON (ip.name='standard_price' AND ip.res_id=CONCAT('product.template,',pt.id) AND ip.company_id=sp.company_id)
                    GROUP BY
                        sm.id,spt.code, sm.date,sm.partner_id,
                        sm.product_id,sm.state,sm.product_uom,sm.date_expected,
                        sm.product_id, sm.picking_id, sm.product_qty,ip.value_float,
...
#        cox gen2 pt.standard_price, stock_journal
...
'''class report_stock_inventory(osv.osv):
    ...
    def init(self, cr):
#        openerp.tools.drop_view_if_exists(cr, 'report_stock_inventory')
        cr.execute("""
CREATE OR REPLACE view report_stock_inventory AS (
    ...
    GROUP BY
        ...
        m.prodlot_id, m.date, m.state, l.usage, m.company_id, pt.uom_id,l.scrap_location,pt.sale_ok
) UNION ALL (
    SELECT
