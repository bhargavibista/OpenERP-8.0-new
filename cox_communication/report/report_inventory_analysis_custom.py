...
select 	all_data.location_id as id
	...
	,coalesce(current_data.qty) as current_product_qty
	,coalesce(previous_data.qty) as previous_product_qty
	,all_data.active as active
    ,coalesce(previous_data2.so_qty) as yesterday_so_qty
    ,coalesce(previous_data1.ext_qty) as yesterday_exchng_qty
from
(
	...
	((SELECT
				...
				coalesce(sum(- m.product_qty * pu.factor / pu2.factor)::decimal, 0.0) as value,
				...
				m.restrict_lot_id, m.date, m.state, l.usage,l.active, l.scrap_location, m.company_id, pt.uom_id, to_char(m.date, 'YYYY'), to_char(m.date, 'MM')
			) UNION ALL (
			    SELECT
...
(
	 ...
	 (SELECT
		...
		stock_move m
			...
			LEFT JOIN product_template pt ON (pp.product_tmpl_id=pt.id)
		    LEFT JOIN stock_location l ON (m.location_id=l.id)
		    WHERE m.state = 'done' and l.usage='internal' and cast(m.date as date)= (SELECT CURRENT_DATE) and (m.origin like 'SO%' or m.origin like 'RO%')
	    GROUP BY
...
(
	 ...
	 (SELECT
		...
		stock_move m
			...
			LEFT JOIN product_template pt ON (pp.product_tmpl_id=pt.id)
		    LEFT JOIN stock_location l ON (m.location_id=l.id)
		    WHERE m.state = 'done' and l.usage='internal' and cast(m.date as date)= (SELECT CURRENT_DATE-1) and (m.origin like 'SO%' or m.origin like 'RO%')
	    GROUP BY
		m.id, m.product_id,  m.location_id,l.name, l.usage
		order by m.date asc
		)
...
(
	 ...
	 (SELECT
		...
		stock_move m
		    ...
		    WHERE m.state = 'done' and l.usage='internal' and cast(m.date as date)= (SELECT CURRENT_DATE-1) and (m.origin like 'RO%')
	    GROUP BY
		m.id, m.product_id, m.location_id,l.name, l.usage,m.origin
		order by m.date asc
		)
		as temp3
...
(
	 ...
	 (SELECT
		...
		stock_move m
		    ...
		    WHERE m.state = 'done' and l.usage='internal' and cast(m.date as date)= (SELECT CURRENT_DATE-1) and (m.origin like 'SO%')
	    GROUP BY
		m.id, m.product_id,m.location_id,l.name, l.usage
		order by m.date asc
		)
...
##cox gen2 changed m.prodlot_id field name to restrict_lot_id
##p.type
##p.type == 'out'
##pt.standard_price