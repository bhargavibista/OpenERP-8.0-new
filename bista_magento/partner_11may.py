...
######## Update Subscription API to upgrade/downgrade service by yogita
	...
	print "subscription data-----",cr,uid,subscription_data
	    ...
	    from_openerp=subscription_data.get('from_openerp','')
            ...
            old_pack_oe_id=product_obj.search(cr,uid,[('product_id','=',old_product_id)])
            print old_pack_oe_id
            if not old_pack_oe_id:
                return json.dumps({'body':{'code':False,'message':'Old Product not found.'}})
            new_pack_oe_id=product_obj.search(cr,uid,[('product_id','=',old_product_id)])
            ...
            try:
                ...
                search_old_policy = partner_policy.search(cr,uid,[('product_id','=',old_product_id),('active_service','=',True),('agmnt_partner','in',partner_id)])
...
<<<<<<< HEAD
                                ...
                                if ast.literal_eval(str(new_policy_result)).has_key('body') and ast.literal_eval(str(new_policy_result)).get('body')['result'] == 4113:
                                   ...
                                   return json.dumps({'body':{'code':False,'message':'Failed to Create service at Playjam End.'}})
                            else:
                                return json.dumps({'body':{'code':False,'message':'Failed to cancel service at Playjam End.'}})
                        else:
                            return json.dumps({'body':{'code':False,'message':'Product not found.'}})
                    else:
...
>>>>>>> 800530b2482f914956877da0bbc6cf2f99ba7d5d
