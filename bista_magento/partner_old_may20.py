...
class res_partner(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context={}):
		...
		ids = [ids]
            playjam_exported=self.browse(cr,uid,ids[0]).playjam_exported
            if playjam_exported:
                ...
                if cust_profile_Id and numberstring:
                    ...
                    if dict_wallet.get('fill_amount')>0.0:
                        ...
                        if response.get('resultCode') == 'Ok':
                            wallet_response=user_auth_obj.wallet_playjam(cr, uid, 'abcd', amount, context=None)
...
#                authorize_net_config.call(cr,uid,config_obj,'VoidTransaction',cust_profile_Id,numberstring,transaction_id)
    ...
    def redeem_gift_card(self,cr,uid,dict,context):
        maerge_invoice_data,result,sale_info,count=[],{},{},0
...
#                                resp_code=1
    ...
    #defining account prepaid revenue or service revenue depending on redemption before the free trial or after free trial correspondingly
                                        ...
                                        print "continueeeeeeeeeeeeeeeeeeeeee",count
                                        count+=1
                                if count==4:
                                    gift_card_reversal=gift_card_obj.api_call_toincomm_reversal(cr,uid,dict_gift_card.get('card_no'),context=context)
                                    result={"body":{ "code":False, "message":"Failed to Redeem Card at Incomm!!!"}}
                            else:
            ...
            gift_card_reversal=gift_card_obj.api_call_toincomm_reversal(cr,uid,dict_gift_card.get('card_no'),context=context)
            result={"body":{'code':False,'message':'Failed to redeem card because %s'%(e)}}
        return json.dumps(result)
#    giftcard        
#    def redeem_gift_card(self,cr,uid,dict,context):
	...
	print "subscription data-----",cr,uid,subscription_data
	    ...
	    from_openerp=subscription_data.get('from_openerp','')
            ...
            try:
                ...
                search_old_policy = partner_policy.search(cr,uid,[('product_id','=',old_product_id),('active_service','=',True),('agmnt_partner','in',partner_id),('product_id','!=',new_product_id)])
...
#                        new_pack_id = product_obj.search(cr,uid,[('default_code','=ilike',new_sku)])
                    new_pack_oe_brw=product_obj.browse(cr,uid,new_product_id)
