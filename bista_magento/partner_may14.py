...
#                authorize_net_config.call(cr,uid,config_obj,'VoidTransaction',cust_profile_Id,numberstring,transaction_id)
                result={'code':False,'message':'Failed to update wallet because %s'%(e)}
            return json.dumps(result)
#    giftcard        
        ...
        if subscription_data:
            ...
            for key, value in dict_exist.iteritems():
                if not value:
                    result={"body":{ 'code':False, 'message':"('%s Not found')"%(key)}}
                    return json.dumps(result)
            customer_id = dict_exist.get('CustomerId')
            new_product_id = dict_exist.get('NewProductId')
            start_date = dict_exist.get('StartDate')
            old_product_id = dict_exist.get('OldProductId')
            partner_obj = self.pool.get('res.partner')
            ...
            if old_pack_oe_id[0]==new_pack_oe_id[0] :
            ...
            try:
                ...
                search_old_policy = partner_policy.search(cr,uid,[('product_id','=',old_pack_oe_id[0]),('active_service','=',True),('agmnt_partner','=',partner_id[0])])
                if search_old_policy:
                    ...
                    if new_prod_categ.parent_id:
                        if (new_prod_categ.parent_id.id not in old_prod_categ_parent) and (new_prod_categ.parent_id.id not in old_prod_categ) and (new_prod_categ.id not in old_prod_categ_parent):
                            return json.dumps({'body':{'code':False,'message':'You can not Upgrade/Downgrade to this new service as it belongs to a different category.'}})
                        ...
                        if (new_prod_categ.id not in old_prod_categ_parent) or (new_prod_categ.id in old_prod_categ):
                            return json.dumps({'body':{'code':False,'message':'You can not Upgrade/Downgrade to this service as it belongs to a different category.'}})
                    ...
                    if ast.literal_eval(str(old_policy_result)).has_key('body') and ast.literal_eval(str(old_policy_result)).get('body')['result'] == 4113:
                        ...
                        cancel_reason = self.pool.get('return.order').additional_info(cr,uid,additional_info)
                        ...
                        if ast.literal_eval(str(new_policy_result)).has_key('body') and ast.literal_eval(str(new_policy_result)).get('body')['result'] == 4113:
                            ...
                            if not subscription_data.get('from_openerp',False):
                                result1=self.pool.get('upgrade.downgrade.policy').create(cr,uid,{
    ...
    def get_transactions_magento(self,cr,uid,dict,context=None):
        ...
        if dict.has_key('CustomerId') and dict.has_key('StartDate') and dict.has_key('EndDate'):
            ...
            if invoice_ids:
                ...
                for each in invoice_ids:
                    inv_brw=invoice_obj.browse(cr,uid,each)
                    product_ids=[line.product_id.id for line in inv_brw.invoice_line]
                    descs=[line.name for line in inv_brw.invoice_line]
	    ...
	    print "start and end daes-----------------",order_start_date,order_end_date
            ...
            if order_ids:
                ...
                for each in order_ids:
                    ...
                    sub={'OrderNo':sale_brw.name,'OrderDate':sale_brw.date_order,'TrackingNo':"",'Total':sale_brw.amount_total,'Subtotal':sale_brw.amount_total,'Discount':sale_brw.amount_total,'Tax':sale_brw.amount_total,'Shippping':0.0}
