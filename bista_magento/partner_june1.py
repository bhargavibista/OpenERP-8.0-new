...
#        dict_old={ 'BillingInfo': {'PaymentProfileId': '','BillingAddress': {'Street1': '581 Telegraph Canyon Rd', 'Street2': '', 'State': 'California', 'Zip': '91910-6436', 'City': 'Chula Vista'}, 'CreditCard': {'CCNumber': '510510493671000', 'CCV': '123', 'ExpDate': '122020'}}, 'FillAmount': 10.0, 'ApiId': '123', 'PaymentType': 'CC', 'CustomerId': 4543,'DBName': 'local_stable_7_test'}
	...
	customer_id=partner_obj.search(cr,uid,[('id','=',int(dict_wallet.get('partner_id')))])
        ...
        if dict_wallet.get('billing_info').get('CreditCard'):
	...
	print "active id.......................................",active_id,type(active_id)
        partner_brw=partner_obj.browse(cr,uid,active_id)
	print "partner browse........",partner_brw
...
#            try:
		...
		print "payemnt profile id found......................................"
...
#        call to create profile if there is no existing payment profile at Authorize end
		...
		print "else part............................"
                #    maxmind passed true for testing purpose
...
#                    maxmind_response = True
		...
		print "cust profile id/////////////////////////////////",cust_profile_Id,numberstring
...
#                partner_obj.cust_profile_payment(cr,uid,active_id,cust_profile_Id,payment_profile_val,exp_date,context)
                if dict_wallet.get('fill_amount')>0.0:
                    amount=dict_wallet.get('fill_amount')
                    if amount<5.0:
                        result={"body":{ 'code':False, 'message':"Minimum Wallet Top-Up should be 5$"}}
                        return json.dumps(result) 
                   # present_amt_playjam_req=user_auth_obj.wallet_playjam(cr, uid,int(dict_wallet.get('partner_id')), 0.0, context=None)
		    present_amt_playjam_req=user_auth_obj.wallet_playjam(cr, uid,active_id, 0.0, context=None)
                    ...
                    if ast.literal_eval(present_amt_playjam_req).get("body") and ast.literal_eval(present_amt_playjam_req).get("body").get('quantity'):
                        ...
                        if amount>500.0 or add_amt>500.0:
                            ...
                            print "x....................................",x
                            result={"body":{ 'code':False, 'message':"You cannot Top-up your wallet more than %s since the max limit in wallet balance is 500$!!!!!!!"%(x)}}
                            return json.dumps(result) 
                    ...
                    prepaid_id_search=prepaid_obj.search(cr,uid,[('card_no','=',ccn[-4:]),('partner_id','=',partner_brw.id)])
                    ...
                    if response.get('resultCode') == 'Ok':
                       # wallet_response=user_auth_obj.wallet_playjam(cr, uid, int(dict_wallet.get('partner_id')), amount, context=None)
...
##function to create an order at a glance through magento online purchase order API Call
    def create_order_magento(self,cr,uid,dict,context):
        vals,dict_exist,line_data,so_data,vals1,vals2,result,street1_inv,street2_inv,street1,street2,payment_profile_id,magento_orderid ={},{},{},{},{},{},{},'','','','','',''
...
#                handling as of now by passing test CC details for hsndling tru orders
                context['tru']=True
                #billing_info={"CreditCard":{"CCNumber":"378282246310005","ExpDate":"122050","CCV":"123"}}
...
#                billing_info={"CreditCard":{"CCNumber":"378282246310005","ExpDate":"122050","CCV":"123"}}
        ...
        try:
            ...
            customer_id=partner_obj.search(cr,uid,[('id','=',int(dict_exist.get('partner_id')))])
            print "customer_idcustomer_id",customer_id
            if not customer_id:
                result={"body":{ 'code':False, 'message':"Customer not found!!!!"}}
                return json.dumps(result)
            shop_id=sale_shop.search(cr,uid,[('name','ilike','company')])
            shop_brw = sale_shop.browse(cr, uid, shop_id[0])
            if shop_brw.warehouse_id and shop_brw.warehouse_id.lot_stock_id:
                location_id= shop_brw.warehouse_id.lot_stock_id.id
            ...
            if state_id and country_id:
                ...
                ship_add=partner_obj.search(cr,uid,[('id','=',int(dict_exist.get('partner_id'))),('street','ilike',street1),('street2','ilike',street2),('city','=',city),('zip','=',zip),('state_id','=',state_id[0]),('country_id','=',country_id)])
		print "ship address.....................",ship_add
                if not ship_add:
...
#                price=each_param.get('Price')
                ...
                if product_id:
                    ...
                    price=product_obj.browse(cr,uid,productid).list_price
                        ...
                        for each_sub_comp in sub_components:
                            ...
                            if product_type=='service':
                                if comp_prod_id in active_services:
                                    ...
                                    return json.dumps(result)
                            sub_comp_data=({'name':each_sub_comp.comp_product_id.name,'qty_uom':float(quantity),'product_id':comp_prod_id,'price':price,'so_line_id':sale_line_id,'product_type':product_type,'uom_id':each_sub_comp.product_id.uom_id.id or 1})
                            print "sub_comp_datasub_comp_data",sub_comp_data
...
######## Update Subscription API to upgrade/downgrade service by yogita
	...
	print "subscription data-----",cr,uid,subscription_data
	    ...
	    from_openerp=subscription_data.get('from_openerp','')
            ...
            old_pack_oe_id=product_obj.search(cr,uid,[('id','=',old_product_id)])
            print old_pack_oe_id
            if not old_pack_oe_id:
                return json.dumps({'body':{'code':False,'message':'Old Product not found.'}})
            new_pack_oe_id=product_obj.search(cr,uid,[('id','=',new_product_id)])
            ...
            try:
            	...
            	search_old_policy = partner_policy.search(cr,uid,[('product_id','=',old_pack_oe_id[0]),('active_service','=',True),('agmnt_partner','in',partner_id)])
#                search_old_policy = partner_policy.search(cr,uid,[('active_service','=',True),('agmnt_partner','in',partner_id)])
...
#                        new_pack_id = product_obj.search(cr,uid,[('default_code','=ilike',new_sku)])
                    new_pack_oe_brw=product_obj.browse(cr,uid,new_pack_oe_id[0])
...
#                        return json.dumps({'body':{'code':False,'message':'You can not Upgrade/Downgrade to the same active subscription.'}})
                    oe_categ_id=product_obj.browse(cr,uid,old_product_id).categ_id
...
#                                    app_id=284
                    ...
                    old_policy_result = user_auth_obj.rental_playjam(cr,uid,user_id,app_id,expiry_epoch1)
...
#                                    app_id=284
			...
			expiry_epoch=int(expiry_epoch)
                        ...
                        if ast.literal_eval(str(new_policy_result)).has_key('body') and ast.literal_eval(str(new_policy_result)).get('body')['result'] == 4113:
                            ...
                            if from_openerp!=True:
                                up_down_id=up_down_obj.create(cr,uid,{
                                ...
                                partner_policy.write(cr,uid,policy_id,{'up_down_id':up_down_id})
                            partner_obj.next_billing_amount(cr,uid,partner_id[0])
                            if policy_id:
	...
	if customer_id:
            ...
            active=self.browse(cr,uid,int(customer_id)).active
            if active != True:
        ...
        if customer_id:
            ...
            cr.execute('select id from res_country_state where code = %s', ('GA',))
	...
	if customer_id and not profile_id:
            ...
            user_profile_ids=self.browse(cr,uid,int(customer_id)).user_profile_ids
...
#                            bill_info.update({'CreditCard':{'CCNumber':each.credit_card_no,}})
    ...
    def get_transactions_magento(self,cr,uid,dict,context=None):
	    ...
	    print "custidddddddddddddddddddddddddddd",cust_id,type(cust_id),start,end
	    ...
	    try:
            ...
            cr.execute("select id from account_invoice where partner_id=%s and date_invoice between '%s' and '%s'"%(cust_id,invoice_start_date,invoice_end_date))
            invoice_ids = filter(None, map(lambda x:x[0], cr.fetchall()))
	    print "invoiceidssssssssssssssssssssssssssss",invoice_ids
            if invoice_ids:
                subscriptions=[]
                invoice_obj=self.pool.get('account.invoice')
                for each in invoice_ids:
    ...
    def get_order_info(self,cr,uid,dict,context=None):
        print "dict----------------",dict,type(dict)
        if dict.has_key('CustomerId') and dict.has_key('StartDate') and dict.has_key('EndDate'):
            cust_id=dict.get('CustomerId')
            start=dict.get('StartDate')
	    ...
	    print "start and end daes-----------------",order_start_date,order_end_date
            ...
            cr.execute("select id from sale_order where partner_id=%s and date_order between '%s' and '%s'"%(cust_id,order_start_date,order_end_date))
            order_ids = filter(None, map(lambda x:x[0], cr.fetchall()))
            print "order_ids---------------",order_ids
            if order_ids:
                order_details=[]
                order_obj=self.pool.get('sale.order')
                for each in order_ids:
                    sale_brw=order_obj.browse(cr,uid,each)
